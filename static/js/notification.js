(() => {
    ...
    async function sync({requestPermission = false} = {}) {
        const fail = message => {
            // Push setup can legitimately be unavailable (for example when
            // VAPID is not configured). Never interrupt the user with an alert
            // during page-load/background synchronization.
            console.warn("[Lucky Push Diagnostic]", message);
            return false;
        };
        ...
        syncPromise = (async () => {
            try {
                ...
                // Keep notification setup failures non-blocking. They are
                // diagnostic conditions, not reasons to interrupt chat usage.
                return false;
            } finally {
                syncPromise = null;
            }
